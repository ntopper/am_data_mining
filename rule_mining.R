...
rules <- apriori(ususalSuspects)
print(inspect(rules))

cat("\n")
cat("\n")
cat("mining all rules where Support = 0.01 and Confidence = 0.90\n")
rules <- apriori(ususalSuspects, parameter = list(support = 0.01, confidence = 0.9))
...
rules.subset <- subset(rules, (rhs %in% paste0("actor=", unique(ususalSuspects$actor))))
